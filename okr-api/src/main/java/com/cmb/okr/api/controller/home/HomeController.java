...
public class HomeController extends BaseController {
	...
	public JsonResponse login(@RequestBody LoginRequest request) {
		return doBusiness((res) -> {
			...
			session.setExpire(OkrConfig.getSessionExpireSeconds() + System.currentTimeMillis());
			// 登录成功保存用户登录状态
	...
	public JsonResponse changePassword(HttpServletRequest request, @RequestBody ChangePasswordRequest prequest) {
		return doBusiness((res) -> {
			...
			String salt = StringUtils.isEmpty(user.getSalt()) ? SecurityUtils.getSalt(user.getYstCode())
					: user.getSalt();

			String pwd = SecurityUtils.getPassphrase(salt, prequest.getNewPassword());
